...
pandoc -f docbook -t markdown_github $docbookFile -o $resultFile

# Unicode symbols were mangled in foo.md. Quick workaround:

iconv -t utf-8 $docbookFile | pandoc -f docbook -t markdown_github | iconv -f utf-8 > $resultFile

# Pandoc inserted hard line breaks at 72 characters. Removed like so:

# pandoc -f docbook -t markdown_github --wrap=none # don't wrap lines at all

# pandoc -f docbook -t markdown_github --columns=120 # extend line breaks to 120